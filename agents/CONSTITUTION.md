...
|------|------------|-----------------|
| **Python version:** `3.13` via `.python-version`. `uv venv` must use `--python-preference only-managed` so uv downloads Python independently of the system. | `.python-version` + `tmux.sh` review + code review | Reject gate |
